...
public class WebQueryProtocol {
    public static String processInput(String theInput) {
        ...
        String apiSecret = MinetraxBukkit.getPlugin().getApiSecret().substring(0, 32);
        ...
        if (!MinetraxBukkit.getPlugin().getApiKey().equals(queryRequestData.api_key)) {
            MinetraxBukkit.getPlugin().getLogger().warning("Error: API Key Mismatch");
            return null;
        }
        ...
        switch (queryRequestData.type) {
            ...
            case "broadcast":
                ...
                break;
            case "command":
                boolean isAllowOnlyWhitelistedCommandsFromWeb = MinetraxBukkit.getPlugin().getIsAllowOnlyWhitelistedCommandsFromWeb();
                List<String> whitelistedCommands = MinetraxBukkit.getPlugin().getWhitelistedCommandsFromWeb();
    ...
    private static String handleUserSay(QueryRequestData queryRequestData) {
        ...
        String messageFormat = MinetraxBukkit.getPlugin().getWebMessageFormat();
    ...
    private static String getPlayerGroups(String uuid) {
        if (!MinetraxBukkit.getVaultPermission().hasGroupSupport()) {
            // System.out.println("No group support found");
            return CryptoUtil.getEncryptedString(MinetraxBukkit.getPlugin().getApiKey(), "{}");
        }
        ...
        String primaryGroup = MinetraxBukkit.getVaultPermission().getPrimaryGroup(null, offlinePlayer);
        String[] allGroups = MinetraxBukkit.getVaultPermission().getPlayerGroups(null, offlinePlayer);
        ...
        // Encrypt
        jsonString = CryptoUtil.getEncryptedString(MinetraxBukkit.getPlugin().getApiKey(), jsonString);
        return jsonString;
    }

    private static boolean handleCommand(String command) {
        try {
            boolean success = Bukkit.getScheduler().callSyncMethod(MinetraxBukkit.getPlugin(), () -> Bukkit.dispatchCommand(Bukkit.getConsoleSender(), command)).get();
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
    ...
    private static String handleStatus() {
        ...
        for (Player player : playerList) {
            ...
            if (MinetraxBukkit.getPlugin().getHasSkinRestorer()) {
